...
require("./lib/env");
const buildPayload = (organisationName: string, projectName: string) => {
    return {
        "meta": {
            "form": "3-10k-grant",
            "schemaVersion": "v1.x",
...
async function run() {
    ...
    return await org.authenticate({
        ...
        .then((oauth: any) => org.apexRest({ oauth: oauth, uri: 'loadData', method: 'POST', body: JSON.stringify(buildPayload('Acme Enterprises', 'Cool Project')) }));
};

run().catch(e => { console.log(`error from salesforce ${e}`); }).then(console.log);
